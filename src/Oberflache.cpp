...
// Aktualisiert die komplette Oberfläche, außer Einstellungen
...
// Wenn Werte aus der Haupttabelle geändert werden, werden diese in die Datenbank geschrieben
// jede Zelle ein Item mit Reihe und Spalte 
	disconnect(WarenTW, 0, 0, 0);
	DB db("db");
...
// Entfernt eine Warengruppe wenn es von dieser keine Ware mehr gibt.
	DB db("db");
	bool b = true;
	for(unsigned i1 = 0;i1<warenVector.size();++i1){
...
// Überträgt Einstellungen in die Datenbank, wenn sich was geändert hat.
	DB db("db");
...
// Überträgt Zieldatum, wenn es sich geändert hat
	DB db("db");
...
// Überträgt Zielzeit, wenn es sich geändert hat
	DB db("db");
...
// Überträgt Zielkommentar, wenn es sich geändert hat
	DB db("db");
